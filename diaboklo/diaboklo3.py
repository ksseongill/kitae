...
# 전투 함수
def si_fight(si_who_monster, si_monster_nowHp, si_choco, si_monster_list):
...
def portal_AA():
    ...
    88   |      |``--..|_ | `;!| | _!|●   | !_| |.'j   |_..!-'|     |    88
    88   |      |    |   |`-,!_|_|..-|____|-..|_||.!-;'  |    |     |    88
    88___|______|____!.,.!,.!,!| | | /    \ | | |,!,.!.,.!..__|_____|____88
    88      |     |    |  |  | |_| / ██████ \||_|| |   |   |    |      | 88 
    88      |     |    |..!-;'i| | !/      \  | | |`-..|   |    |      | 88
...
def battle(si_choco):
    ...
    # 전투 함수 호출
    si_fight(si_who_monster, si_monster_Hp[0], si_choco, si_monster_list)
...
def print_map(pm_maze, si_choco):
    ...
    potal_a = random.randrange(1, len(pm_maze) - 1)
    potal_b = random.randrange(1, len(pm_maze) - 1)
